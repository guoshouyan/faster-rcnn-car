...
NETS = {'vgg16': ('VGG16',
	...
	'vgg1024':('VGG_CNN_M_1024',
        ...
        'zf': ('ZF',
                'zf_faster_rcnn_iter_220000.caffemodel')}
...
def vis_detections2(im,image_name,gtBox):
...
if __name__ == '__main__':
    ...
    im_dir = '../lib/datasets/VOCdevkit-matlab-wrapper/fn.txt'
    f = open(im_dir, "r")
    line = f.readline()
    data = line.strip().split(' ')
    im_name=data[0]
    gtBox=[map(float,data[1:5])]
    try:
        while True:
            line = f.readline()
            data = line.strip().split(' ')
            if im_name==data[0]:
                gtBox.append(map(float,data[1:5]))
            else:
                print 'Demo for data/demo/{}'.format(im_name)
                im_file = os.path.join(cfg.ROOT_DIR, '../kitti/train', 'image_2', im_name+'.png')
                im = cv2.imread(im_file)
                vis_detections2(im,im_name,gtBox)
                plt.savefig('output/'+im_name+'.png')
                #plt.show()
                plt.close()
                im_name=data[0]
                gtBox=[map(float,data[1:5])]
