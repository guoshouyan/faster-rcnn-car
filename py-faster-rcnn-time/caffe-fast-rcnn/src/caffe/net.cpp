...
#include "caffe/util/benchmark.hpp" //guo
...
int Net<Dtype>::ForwardFromTo2() {
  ...
  Dtype initial_loss;
  Forward(vector<Blob<Dtype>*>(), &initial_loss);
  LOG(INFO) << "Initial loss: " << initial_loss;
  LOG(INFO) << "Performing Backward";
  Backward();

  int FLAGS_iterations=10;

  const vector<shared_ptr<Layer<Dtype> > >& layers = layers_;
  const vector<vector<Blob<Dtype>*> >& bottom_vecs = bottom_vecs_;
  const vector<vector<Blob<Dtype>*> >& top_vecs = top_vecs_;
  const vector<vector<bool> >& bottom_need_backward = bottom_need_backward_;
