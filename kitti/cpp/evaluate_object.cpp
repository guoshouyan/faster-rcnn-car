...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  FILE *mapf = fopen((dir_name + "/" + file_name + "_map" + ".txt").c_str(), "w");
  double sum1 = 0, sum2 = 0, sum3 = 0, ave1 = 0, ave2 = 0, ave3 = 0;
  for (int32_t i = 0; i < (int)N_SAMPLE_PTS; i++)
  {
	sum1 += vals[0][i];
	sum2 += vals[1][i];
  ...
  ave1 = sum1 / (int)N_SAMPLE_PTS;
  ave2 = sum2 / (int)N_SAMPLE_PTS;
  ave3 = sum3 / (int)N_SAMPLE_PTS;
  fprintf(mapf, "%s %f %f %f\n", "map", ave1, ave2, ave3);
  fclose(mapf); 
  // create png + eps
...
bool eval(string result_sha,Mail* mail){
  ...
  string gt_dir         = "/mnt/data/KITTI/training/label_2";
